...
export class VideoController {
  ...
  /**
   * GET /api/videos/validate-url
   * Validate any supported platform URL (YouTube, TikTok, Instagram, etc.)
   */
  static async validateVideoUrl(c: Context) {
    const url = c.req.query("url");
    VideoController.logRequest(c, "VALIDATE_VIDEO_URL", { url });
    ...
    try {
      ...
      // Proxy to worker (has yt-dlp installed)
      const workerUrl = process.env.WORKER_URL;
      if (workerUrl) {
        try {
          const workerRes = await fetch(
            `${workerUrl}/validate-url?url=${encodeURIComponent(url)}`,
            { signal: AbortSignal.timeout(30000) }
          );
          const data = await workerRes.json() as any;
          return c.json(data);
        } catch (proxyErr: any) {
          console.error("[VIDEO CONTROLLER] Worker proxy failed for validate-url:", proxyErr?.message);
        }
      }

      const result = await YtDlpService.validateUrl(url);
      return c.json({ ...result, platform: platformInfo.platform });

    } catch (error: any) {
      console.error(`[VIDEO CONTROLLER] VALIDATE_VIDEO_URL error:`, error);
      return c.json({ valid: false, error: error?.message || "Failed to validate URL" });
    }
  }
   ...
   */
  static async submitVideoUrl(c: Context) {
    VideoController.logRequest(c, "SUBMIT_VIDEO_URL");

    try {
      const body = await c.req.json();
      const { projectId, videoUrl, workspaceSlug, workspaceId, config } = body;
      const user = c.get("user") as { id: string };
      ...
      if (!platformInfo) {
        return c.json({ error: "Unsupported platform URL" }, 400);
      }

      const { WorkspaceModel } = await import("../models/workspace.model");
      const member = await WorkspaceModel.getMemberByUserAndWorkspace(user.id, workspaceId);
      if (!member) return c.json({ error: "You don't have access to this workspace" }, 403);

      // Get video info — proxy to worker if available
      let videoInfo: import("../services/ytdlp.service").VideoInfo;
      try {
        const workerUrl = process.env.WORKER_URL;
        if (workerUrl) {
          try {
            const workerRes = await fetch(
              `${workerUrl}/validate-url?url=${encodeURIComponent(videoUrl)}`,
              { signal: AbortSignal.timeout(30000) }
            );
            const data = await workerRes.json() as any;
            if (data.valid && data.videoInfo) {
              ...
              return c.json({ error: data.error || "Failed to retrieve video information." }, 400);
            }
          } catch {
            videoInfo = await YtDlpService.getVideoInfo(videoUrl);
          }
        } else {
          videoInfo = await YtDlpService.getVideoInfo(videoUrl);
        }
      } catch (error: any) {
        return c.json({ error: error.message || "Failed to retrieve video information." }, 400);
      }
      ...
      const uploadValidation = canUploadVideo(planConfig, videoInfo.duration, 0, minutesBalance.minutesRemaining, effectiveDuration);
      if (!uploadValidation.allowed) {
        const canUpgrade = plan === "free" || plan === "starter";
        return c.json({
          error: uploadValidation.message,
          ...
          upgrade: uploadValidation.upgrade,
          upgradeRequired: canUpgrade && uploadValidation.reason === "VIDEO_TOO_LONG",
          recommendedPlan: canUpgrade && uploadValidation.reason === "VIDEO_TOO_LONG" ? (plan === "free" ? "starter" : "pro") : undefined,
          currentLimit: uploadValidation.reason === "VIDEO_TOO_LONG" ? formatDuration(planConfig.limits.videoLength) : undefined,
          minutesRemaining: uploadValidation.reason === "INSUFFICIENT_MINUTES" ? minutesBalance.minutesRemaining : undefined,
          minutesNeeded: uploadValidation.reason === "INSUFFICIENT_MINUTES" ? uploadValidation.minutesWillBeDeducted : undefined,
        }, uploadValidation.reason === "INSUFFICIENT_MINUTES" ? 402 : 400);
      ...
      const videoRecord = await VideoModel.create({
        ...
        workspaceId,
        userId: user.id,
        sourceType: platformInfo.platform as any,
        sourceUrl: videoUrl,
        title: videoInfo.title,
        expiresAt: getVideoExpiryDate(plan),
      });
      ...
      const minutesToDeduct = calculateMinuteConsumption(effectiveDuration);
      await MinutesModel.deductMinutes({ workspaceId, userId: user.id, videoId, amount: minutesToDeduct, type: "upload" });

      if (config) {
        const { VideoConfigModel } = await import("../models/video-config.model");
        await VideoConfigModel.upsert(videoId, nanoid(), {
          skipClipping: config.skipClipping ?? false,
          clipModel: config.clipModel ?? "ClipBasic",
          genre: config.genre ?? "Auto",
          clipDurationMin: config.clipDurationMin ?? 15,
        ...
        return c.json({ message: "Video submitted for processing", video: { ...videoRecord, status: "downloading" }, videoInfo, platform: platformInfo.platform }, 201);
      }
      ...
      return c.json({
        ...
        redirectUrl: workspaceSlug ? `/${workspaceSlug}/configure/${videoId}` : `/configure/${videoId}`,
      }, 201);

    } catch (error) {
      console.error(`[VIDEO CONTROLLER] SUBMIT_VIDEO_URL error:`, error);
      return c.json({ error: error instanceof Error ? error.message : "Failed to submit video" }, 500);
    }
  }
}
