...
const ViralClipSchema = z.object({
  clips: z.array(
    z.object({
      ...
      introTitle: z.string().optional().default("").transform(val => val.replace(/[\p{Emoji_Presentation}\p{Extended_Pictographic}]/gu, "").trim().slice(0, 30)).describe("A short, punchy intro title (max 4-5 words, under 30 characters, NO emojis or icons - plain text only) to display in the first 3 seconds of the video - MUST fit on 2 lines max when rendered in large text. Should hook viewers immediately. Use empty string if not needed."),
...
- Create a short, punchy intro title (4-5 words max, under 30 characters) for each clip
...
- Do NOT include any emojis or icons in the intro title - plain text only
